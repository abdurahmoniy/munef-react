...
export default function Order({ pdata }) {
    const { id } = useParams();
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      try {
        const response = await fetch(`https://api.munef.uz/api/order/store/${productId}`, {
          method: 'POST',
          headers: {
    ...
    <div>
      ...
      <div class="body">
        <div class="breadcrumbs">
            <Link to='/rus' class="bc_item1">Главный</Link>
            <img class="bc_item2" src={slash} alt=""/>
            <div class="bc_item3">Контакт</div>
        </div>
        <div className="selected_product">
          <div className="sp_detail">
            <div className="sp_title">Ваш продукт:</div>
            <div className="sp_name">{product.rus_name}</div>
            <div class="narx">
        ...
        <div class="contact">
            <div class="boglanish">
                <h2 class="contact-name">Контактная информация</h2>
                <div class="desc3">Запишите свой личный номер и имя и мы свяжемся с вами</div>
                <div class="call">
                ...
                <div class="loc">
                    ...
                    <p class="contact-item">Улица Мукими, город Ташкент </p>
                </div>
